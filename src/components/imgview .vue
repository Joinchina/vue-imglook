<template>
  <div id="modelbox" v-show="showmodel">
        ...
        <ul>
          <li id="li" v-for="(img,index) in passimg" :key="index">
            <div id="imgbox" style="transition:width 0.2s,height 0.2s;">
              <img :src="img.src" @mousedown="opepicture($event,realnum)" alt=""  draggable="false" style="transition:width 0.2s,height 0.2s;">
      ...
      <div id="operation">
        <div id="leftimg" @click="leftbtn($event)"><img src="static/imgmoudel/pic9.png" alt=""></div>
        <div id="bigimg" @click="setimgsize(realnum,'big',$event)"><img src="static/imgmoudel/pic4.png" alt=""></div>
        <div id="playimg" @click="playing($event)" v-if="!playpause"><img src="static/imgmoudel/pic3.png" alt=""></div>
        <div id="pause"  @click="pause($event)" v-else><img src="static/imgmoudel/pic5_stop.png" alt=""></div>
        <div id="smallimg" @click="setimgsize(realnum,'small',$event)"><img src="static/imgmoudel/pic6.png" alt=""></div>
        <div id="rightimg" @click="rightbtn($event)"><img src="static/imgmoudel/pic7.png" alt=""></div>
      </div>
  </div>
</template>
  ...
  methods:{
      ...
      setimg(n){
       ...
       // 设置当前ul的宽高
        ...
        this.title=this.passimg[n].name //显示图片名字
    ...
    // 左右滑动的实现
      leftbtn(event){
        ...
        ulArr.style.transform="translateX("+(-this.cliwidth*this.realnum)+"px)" 
        this.title=this.imgnumber[this.realnum].name //显示图片名字
        this.wheelimg(this.realnum)       
      },
      rightbtn(event){
        //取消冒泡
        ...
        document.onmouseup=function(){
          ...
          if(imgwidth>liwidth && imgheight>liheight){
              ...
              if(xjl<(liwidth-imgwidth/2)){
                imgbox.style.left=(liwidth-imgwidth/2)+"px";
              };
              if(yjl>(imgheight/2)){
               imgbox.style.top=(imgheight/2)+86+"px";
              };
...
.viewshow ul li .close{
 ...
 top:-14px;
 right:-14px;
 background-image: url("../../static/imgmoudel/icon-close.png");
 width: 28px;
 height: 28px;
}
.headimg{
